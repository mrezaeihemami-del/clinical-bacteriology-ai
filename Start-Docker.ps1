...
function Ensure-LocalEnv {
    ...
    if (-not (Test-Path -LiteralPath $Path)) {
        ...
        $candidateEnvs = @(
            @{
                Version = "v6"
                Path = Join-Path $parent "clinical-bacteriology-ai-assistant-docker-only-fixed-v6\.env"
            },
            @{
                Version = "v5"
                Path = Join-Path $parent "clinical-bacteriology-ai-assistant-docker-only-fixed-v5\.env"
            },
            @{
                Version = "v4"
                Path = Join-Path $parent "clinical-bacteriology-ai-assistant-docker-only-fixed-v4\.env"
            }
        )

        foreach ($candidate in $candidateEnvs) {
            if (Test-Path -LiteralPath $candidate.Path) {
                Copy-Item -LiteralPath $candidate.Path -Destination $Path -Force
                Write-Host "Imported the existing $($candidate.Version) .env file." -ForegroundColor Green
                break
            }
        }
    }
...
try {
    ...
    $script:LogPath = Join-Path $ProjectRoot ("logs\startup-v7-" + (Get-Date -Format "yyyyMMdd-HHmmss") + ".log")
    "CBAI v7 startup - $(Get-Date -Format o)" | Set-Content -LiteralPath $script:LogPath -Encoding UTF8

    Write-Host ""
    Write-Host "Clinical Bacteriology AI Assistant - fixed v7" -ForegroundColor Green
    ...
    $null = Invoke-Compose -Arguments @("up", "-d", "--force-recreate", "--remove-orphans")
